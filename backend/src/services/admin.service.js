...
export const fetchChartData = async () => {
  ...
  const monthlyData = await Place.findAll({
    ...
    where: {
      created_at: {
        [Op.gte]: new Date(new Date().setMonth(new Date().getMonth() - 9)),
      },
    },
    group: [
...
export const approvePlaceById = async (placeId, adminId) => {
  ...
  if (place.submitter) {
    try {
      await earnPoints({
        userId:      place.submitter.id,
        action:      "place_approved",
        description: `Your place "${place.name}" was approved!`,
    ...
    try {
      ...
      if (N) await N.create({
        ...
        type:     "place_approved",
        message:  `Your place "${place.name}" has been approved and is now live! You earned 50 bonus points 🎉`,
        is_read:  false,
      });
...
export const updatePlaceById = async (placeId, fields, files) => {
  ...
  if (files && files.length > 0) {
    ...
    catch { existing = place.image ? [place.image] : []; }
...
// ── FIX: associated records पहिले delete गर्नुस् ─────────────────────────────
export const deletePlaceById = async (placeId) => {
  const place = await Place.findByPk(placeId);
  if (!place) return { deleted: false };

  // PlaceVisit, Notification, PostReport — foreign key भएकाले पहिले delete
  try { await PlaceVisit.destroy({ where: { place_id: placeId } }); } catch (e) { console.warn("PlaceVisit delete:", e.message); }
  try { await Notification.destroy({ where: { place_id: placeId } }); } catch (e) { console.warn("Notification delete:", e.message); }
  try { await PostReport.destroy({ where: { place_id: placeId } }); } catch (e) { console.warn("PostReport delete:", e.message); }
  ...
  if (status === "resolved" && report.user_id) {
    try {
      await earnPoints({
        ...
        description: "Report reviewed and resolved — thank you for helping!",
        referenceId: report.id,
      });
    } catch (e) { console.warn("Reporter points failed:", e.message); }
  }
