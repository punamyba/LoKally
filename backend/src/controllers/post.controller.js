...
// ─────────────────────────────────────────────────────────────────────────────
//  HELPER — attach has_liked / is_bookmarked flags per user
// ─────────────────────────────────────────────────────────────────────────────
async function attachUserFlags(posts, userId) {
  if (!userId || posts.length === 0) return posts.map(p => p.toJSON ? p.toJSON() : p);

  const ids = posts.map(p => p.id);

  const [likes, bookmarks] = await Promise.all([
    PostLike.findAll({ where: { post_id: { [Op.in]: ids }, user_id: userId } }),
    PostBookmark.findAll({ where: { post_id: { [Op.in]: ids }, user_id: userId } }),
  ]);
...
// ─────────────────────────────────────────────────────────────────────────────
//  GET /api/posts  — paginated feed, hidden posts excluded
// ─────────────────────────────────────────────────────────────────────────────
...
export const createPost = async (req, res) => {
  try {
    ...
    return res.status(201).json({
      success: true,
      data: { ...full.toJSON(), has_liked: false, is_bookmarked: false },
    });
  } catch (err) {
...
// ─────────────────────────────────────────────────────────────────────────────
//  POST /api/posts/:id/like  — toggle like/react
// ─────────────────────────────────────────────────────────────────────────────
  ...
  try {
    ...
    if (existing) {
      if (existing.react_type === reactType) {
        ...
        // Different react → change type, count stays same
...
export const addComment = async (req, res) => {
  try {
    ...
    // Only increment top-level comments
