...
import { Upload, MapPin, CheckCircle, X, Images, Plus, Tag, Star } from "lucide-react";
...
function FlyToLocation({ target }: { target: [number, number] | null }) {
  const map = useMap();
  if (target) map.flyTo(target, 14, { duration: 1.0 });
  return null;
}
...
export default function AdminAddPlace() {
  ...
  const handleLocationPick = (lat: number, lng: number) => {
    ...
    setTimeout(() => setGeoTarget(null), 1500);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res.success) {
      if (selectedTags.length > 0 && res.data?.id) {
        try { await axiosInstance.put(`/places/${res.data.id}/tags`, { tags: selectedTags }); }
        catch {}
      }
      setSuccess(true);
    ...
    <div className="aap-root">
      ...
      <div className="aap-body">
        ...
        <div className="aap-form-wrap">
          {success && (
            <div className="aap-success">
              <CheckCircle size={15} strokeWidth={2.5} /> Place added and is now live on the map!
            </div>
          )}
          {error && (
            <div className="aap-error"><X size={15} strokeWidth={2.5} /> {error}</div>
          )}
          ...
          <form className="aap-form" onSubmit={handleSubmit}>
            ...
            {/* Featured toggle */}
            <div
              className={`aap-featured-toggle ${isFeatured ? "aap-featured-toggle--on" : ""}`}
              onClick={() => setIsFeatured(v => !v)}>
              <div className="aap-featured-toggle-left">
                <div className={`aap-featured-icon ${isFeatured ? "aap-featured-icon--on" : ""}`}>
                  <Star size={16} strokeWidth={2} fill={isFeatured ? "currentColor" : "none"} />
                </div>
                <div>
                  <div className="aap-featured-label">Mark as Featured</div>
                  <div className="aap-featured-desc">
                    Featured places appear on the Home page and show as golden pins on the map
                  </div>
                </div>
              </div>
              <div className={`aap-toggle-switch ${isFeatured ? "aap-toggle-switch--on" : ""}`}>
                <div className="aap-toggle-knob" />
              </div>
            </div>
            ...
            <div className="aap-field">
              ...
              <div className="aap-tag-input-row">
                <div className="aap-tag-input-wrap">
                  <input className="aap-input" value={tagInput}
            ...
            <div className="aap-field">
              <div className="aap-photo-header">
                <label className="aap-label"><Images size={14} strokeWidth={2} /> Photos</label>
              ...
              {imagePreviews.length > 0 ? (
                <div className="aap-photos-grid">
                  ...
                  {imageFiles.length < MAX_PHOTOS && (
                    <div className="aap-photo-add-more" onClick={() => fileRef.current?.click()}>
                      <Plus size={20} strokeWidth={2} /><span>Add</span>
                    </div>
                  )}
                </div>
              ) : (
                <div className="aap-upload-box"
                  ...
                  <div className="aap-upload-sub">Up to {MAX_PHOTOS} photos · JPG, PNG, WEBP</div>
                </div>
              )}
            ...
            <div className="aap-map-hint">
              ...
              Search a location above the map, or click / drag the pin to set location
            </div>

            <button className="aap-submit" type="submit" disabled={loading}>
              {loading ? "Adding..." : (
                <><CheckCircle size={16} strokeWidth={2.5} /> Add Place {isFeatured && "⭐"}</>
              )}
            </button>
          </form>
        </div>
        ...
        <div className="aap-map-wrap">
          <div className="aap-map-container">

            {/* Search overlay — full width, zoom buttons are bottomright now */}
            <div className="aap-map-search-overlay">
              <MapLocationSearch onPick={handleLocationPick} />
            </div>

            <MapContainer
              center={[pin.lat, pin.lng]} zoom={7}
              style={{ height: "100%", width: "100%" }}
              zoomControl={false}>
              <TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />
              {/* Zoom buttons moved to bottomright — search bar full width hola */}
              <ZoomControl position="bottomright" />
              <MapPicker pin={pin} setPin={setPin} />
              <FlyToLocation target={geoTarget} />
            </MapContainer>
