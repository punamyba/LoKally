...
function DetailsExtraInfo({ placeId, refreshKey }: { placeId: number; refreshKey: number }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true);
      try {
        const [condRes, tagRes] = await Promise.all([
          axiosInstance.get(`/places/${placeId}/conditions`).catch(() => ({ data: { data: null } })),
          axiosInstance.get(`/places/${placeId}/tags`).catch(() => ({ data: { data: { tags: [] } } })),
        ]);

        if (!mounted) return;

        setConditions(condRes.data?.data || null);
  ...
  if (loading) {
    return <div className="apl-extra-loading">Loading extra details...</div>;
  }

  return (
    <div className="apl-extra-wrap">
      <div className="apl-detail-meta" style={{ marginTop: "12px" }}>
        <div className="apl-detail-meta-item">
          <div className="apl-detail-meta-label">Trail Condition</div>
          <div className="apl-detail-meta-val">{conditions?.trail_condition || "—"}</div>
        </div>

        <div className="apl-detail-meta-item">
          <div className="apl-detail-meta-label">Road Condition</div>
          <div className="apl-detail-meta-val">{conditions?.road_condition || "—"}</div>
        </div>

        <div className="apl-detail-meta-item">
          <div className="apl-detail-meta-label">Difficulty</div>
          <div className="apl-detail-meta-val">{conditions?.difficulty || "—"}</div>
        </div>
      </div>

      <div className="apl-detail-meta" style={{ marginTop: "12px" }}>
        <div className="apl-detail-meta-item">
          <div className="apl-detail-meta-label">Best Time</div>
          <div className="apl-detail-meta-val">{conditions?.best_time || "—"}</div>
        </div>

        <div className="apl-detail-meta-item" style={{ gridColumn: "span 2" }}>
          <div className="apl-detail-meta-label">Admin Note</div>
          <div className="apl-detail-meta-val" style={{ display: "block" }}>
            {conditions?.note || "—"}
          </div>
        </div>
      </div>

      <div className="apl-cond-section" style={{ marginTop: "14px" }}>
        <div className="apl-cond-label"><Tag size={13} strokeWidth={2} /> Saved Tags</div>
        {tags.length > 0 ? (
          <div className="apl-selected-tags">
            {tags.map((t) => (
              <span key={t} className="apl-selected-tag">{t}</span>
            ))}
          </div>
        ) : (
          <div className="apl-no-tags">No tags saved yet.</div>
        )}
      </div>
    </div>
  );
  ...
  useEffect(() => {
    axiosInstance.get(`/places/${placeId}/conditions`)
      .then(res => {
        if (res.data?.data) {
          setConditions({
            trail_condition: res.data.data.trail_condition || "",
            road_condition: res.data.data.road_condition || "",
            best_time: res.data.data.best_time || "",
          ...
          {suggestions.length > 0 && (
            <div className="apl-tag-suggestions">
              {suggestions.map(s => (
                <div
                  key={s}
                  className="apl-tag-suggestion-item"
                  onClick={() => addTag(s)}
                >
                  {s}
                </div>
      ...
      <div className="apl-detail-modal" onClick={e => e.stopPropagation()}>
        ...
        {photos.length > 0 ? (
          <div className="apl-detail-hero">
            ...
            {photos.length > 1 && (
              <>
                <button
                  className="apl-gallery-arrow apl-gallery-arrow--left"
                  onClick={() => setActivePhoto(v => v === 0 ? photos.length - 1 : v - 1)}
                >
                  <ChevronLeft size={20} />
                </button>
                <button
                  className="apl-gallery-arrow apl-gallery-arrow--right"
        ...
        {tab === "details" && (
          <div className="apl-detail-body">
            ...
            <DetailsExtraInfo placeId={place.id} refreshKey={refreshKey} />

            <div className="apl-detail-footer">
              <button
                className="apl-detail-del-btn"
                onClick={() => { onClose(); onDelete(place); }}
              >
        ...
        {tab === "conditions" && (
          <div className="apl-tab-content-wrap">
            <ConditionsTab placeId={place.id} onSaved={handleSaved} />
          </div>
        )}

        {tab === "tags" && (
          <div className="apl-tab-content-wrap">
            <TagsTab placeId={place.id} onSaved={handleSaved} />
          </div>
        )}
      </div>
    </div>
