...
import {
  ...
  ChevronRight, X, RefreshCw, AlertTriangle, Images,
} from "lucide-react";
...
export default function AdminPending() {
  ...
  const loadVisits = useCallback(async () => {
    ...
    try {
      const r = await axiosInstance.get("/places/admin/visits", {
      ...
      notify(false, "Failed to load visits.");
    }
  ...
  }, [tab, visitFilter, loadVisits]);

  const handleApprove = async (visitId: number) => {
    setActing("approve");
    try {
      await axiosInstance.patch(`/places/admin/visits/${visitId}/approve`);
      setVisits(prev => prev.filter(v => v.id !== visitId));
      setVisitTotal(t => t - 1);
      setSelectedVisit(null);
      notify(true, "Visit approved!");
    } catch {
      notify(false, "Failed to approve.");
    }
    setActing(null);
  };
  ...
  const handleReject = async (visitId: number) => {
    setActing("reject");
    try {
      await axiosInstance.patch(`/places/admin/visits/${visitId}/reject`, { reason: rejectMsg });
      setVisits(prev => prev.filter(v => v.id !== visitId));
      setVisitTotal(t => t - 1);
      setSelectedVisit(null);
      setShowReject(false);
  ...
  const handleUnapprove = async (visitId: number) => {
    setActing("reject");
    try {
      await axiosInstance.patch(`/places/admin/visits/${visitId}/unapprove`);
      setVisits(prev => prev.filter(v => v.id !== visitId));
      setVisitTotal(t => t - 1);
      setSelectedVisit(null);
      notify(true, "Visit removed.");
    } catch {
      notify(false, "Failed to remove visit.");
    }
    setActing(null);
  };
  ...
  useEffect(() => {
    axiosInstance
      .get("/places/admin/visits", { params: { status: "pending", page: 1, limit: 1 } })
      .then(r => {
        if (r.data.success) setPendingVisitCount(r.data.total);
      })
      .catch(() => {});
  }, []);
  ...
  return (
    <div className="apd-root">
      ...
      <div className="apd-tabs">
        <button
          className={`apd-tab ${tab === "places" ? "apd-tab--on" : ""}`}
        ...
        <button
          className={`apd-tab ${tab === "visits" ? "apd-tab--on" : ""}`}
                ...
                <div key={v.id} className="apd-visit-card" onClick={() => setSelectedVisit(v)}>
                  ...
                  <div className="apd-visit-info">
                    ...
                    <div className="apd-visit-meta">
                      ...
                      {v.experience && (
                        <span className="apd-visit-exp">
                          "{v.experience.slice(0, 60)}{v.experience.length > 60 ? "…" : ""}"
                        </span>
                      )}
                    </div>
                  </div>
        ...
        <div className="apd-overlay" onClick={closeVisitModal}>
          <div className="apd-visit-modal" onClick={e => e.stopPropagation()}>
            <div className="apd-modal-head">
              ...
              <button className="apd-modal-close" onClick={closeVisitModal}>
                <X size={16} />
              </button>
            </div>
            ...
            <div className="apd-modal-body">
              ...
              {selectedVisit.status === "pending" && (
                <div className="apd-modal-actions">
                  <button
                    className="apd-act-btn apd-act-btn--approve"
                  ...
                  {!showReject ? (
                    <button
                      className="apd-act-btn apd-act-btn--reject"
              ...
              {selectedVisit.status === "approved" && (
                <div className="apd-modal-actions">
                  <button
                    className="apd-act-btn apd-act-btn--reject"
                    onClick={() => handleUnapprove(selectedVisit.id)}
                    disabled={!!acting}
                  >
                    <X size={14} />
                    {acting === "reject" ? "Removing…" : "Remove Approval"}
                  </button>
                </div>
              )}
            </div>
