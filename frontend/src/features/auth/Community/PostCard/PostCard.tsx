...
type Liker = { first_name: string; last_name: string; react_type: string };

const REACT_EMOJI: Record<string, string> = {
  like: "👍", love: "❤️", wow: "😮", haha: "😂", sad: "😢", angry: "😡",
};
...
export default function PostCard({ post, currentUserId, isAdmin, onDelete, onHide }: Props) {
  ...
  // Filtered likers by react type
  const filteredLikers = likersFilter === "all"
    ? likers
    : likers.filter((l) => l.react_type === likersFilter);

  // Count per react type
  ...
  const handleDownload = async () => {
    ...
    try {
      const res = await fetch(getImageUrl(images[0]));
  ...
  const submitComment = async () => {
    ...
    try {
      ...
      if (r.success) {
        if (replyTo) {
          setComments((prev) => prev.map((c) => c.id === replyTo.id ? { ...c, replies: [...(c.replies ?? []), r.data] } : c));
        } else {
    ...
    <>
      <article className={`pc ${isHidden ? "pc--hidden" : ""}`}>
        ...
        {images.length > 0 && (
          <div className="pc-media">
            {images.length === 1 && (
              <img className="pc-img pc-img--single" src={getImageUrl(images[0])} alt=""
                loading="lazy" decoding="async"
                onClick={() => setLightboxIdx(0)} style={{ cursor: "pointer" }} />
            )}
            {images.length === 2 && (
              <div className="pc-img-grid pc-img-grid--2">
                {images.map((img, i) => (
                  <img key={i} src={getImageUrl(img)} alt="" loading="lazy" decoding="async"
            ...
            {images.length === 3 && (
              <div className="pc-img-grid pc-img-grid--3">
                <img className="pc-img-big" src={getImageUrl(images[0])} alt=""
                  loading="lazy" decoding="async"
                  onClick={() => setLightboxIdx(0)} style={{ cursor: "pointer" }} />
          ...
          <div className="pc-stats">
            {likesCount > 0 && (
              <div className="pc-stat-likes-wrap">
                <button className="pc-stat-likes" onClick={handleShowLikers}>
          ...
          <div className="pc-share-wrap">
            ...
            {showShare && (
              <div className="pc-share-menu" onMouseLeave={() => setShowShare(false)}>
                ...
                <button className="pc-share-item pc-share-item--wa" onClick={handleWhatsApp}><span>💬</span> WhatsApp</button>
                <button className="pc-share-item pc-share-item--fb" onClick={handleFacebook}><span>📘</span> Facebook</button>
                {images.length > 0 && (
                  <button className="pc-share-item" onClick={handleDownload}><Download size={13} /> Download</button>
                )}
              </div>
            )}
          </div>
                ...
                <>
                  ...
                  <div className="pc-report-opts">
                    {REPORT_REASONS.map((r) => (
                      <button key={r} className={`pc-report-opt ${reportReason === r ? "pc-report-opt--on" : ""}`}
                        onClick={() => setReportReason(r)}>{r}</button>
                    ))}
                  </div>
                  <button className="pc-report-submit" disabled={!reportReason} onClick={submitReport}>Submit Report</button>
                </>
              )}
            </div>
          </div>
        ...
        <div className="pc-overlay" onClick={() => setShowLikers(false)}>
          <div className="pc-likers-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="pc-likers-header">
              ...
              <button className="pc-modal-close" onClick={() => setShowLikers(false)}>
            ...
            <div className="pc-likers-list">
              {likersLoading ? (
                <div className="pc-likers-loading">
                ...
                filteredLikers.map((l, i) => (
                  <div key={i} className="pc-likers-row">
                    <div className="pc-likers-av-wrap">
                      ...
                      <span className="pc-likers-react-badge">{REACT_EMOJI[l.react_type] ?? "👍"}</span>
                    </div>
                    <span className="pc-likers-name">{l.first_name} {l.last_name}</span>
                  </div>
                ))
              )}
            </div>
      ...
      {lightboxIdx !== null && (
        <div className="pc-lightbox" onClick={() => setLightboxIdx(null)}>
          <button className="pc-lightbox-close" onClick={() => setLightboxIdx(null)}>
