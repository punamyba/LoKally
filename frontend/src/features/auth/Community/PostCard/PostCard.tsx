...
export default function PostCard({ post, currentUserId, isAdmin, onDelete, onHide }: Props) {
  ...
  const handleWhatsApp = () => {
    const text = encodeURIComponent(
      `${post.caption ? post.caption.slice(0, 80) + " — " : ""}Check this post on LoKally Nepal: ${postUrl}`
    );
  ...
  const submitReport = async () => {
    ...
    try {
      ...
      setReported(true);
      setTimeout(() => setShowReport(false), 2200);
    } catch {}
  };
    ...
    <article className={`pc ${isHidden ? "pc--hidden" : ""}`}>
      ...
      {images.length > 0 && (
        <div className="pc-media">
          {images.length === 1 && (
            <img
              className="pc-img pc-img--single"
            ...
            <div className="pc-img-grid pc-img-grid--4">
              {images.slice(0, 4).map((img, i) => (
                <div key={i} className="pc-img-wrap">
                  ...
                  />
                  {i === 3 && images.length > 4 && (
                    <div className="pc-img-more">+{images.length - 4}</div>
                  )}
                </div>
              ))}
            </div>
                ...
                <div className="pc-likers-popup">
                  ...
                  <div className="pc-likers-body">
                    <div className="pc-liker-row">
                      <span>👍❤️😮😂😢😡</span>
                      <span className="pc-liker-count">
                        {likesCount} total reaction{likesCount !== 1 ? "s" : ""}
                      </span>
                    </div>
      ...
      <div className="pc-actions">
        <div
          ...
          onMouseEnter={() => {
            ...
            reactTimer.current = setTimeout(() => setShowReacts(true), 380);
          }}
          onMouseLeave={() => {
            if (reactTimer.current) clearTimeout(reactTimer.current);
            reactTimer.current = setTimeout(() => setShowReacts(false), 280);
          }}
        >
          <button
          ...
          >
            {liked ? (
              <span className="pc-action-emoji">{REACTS[likeType]?.emoji}</span>
            ) : (
                ...
                <div className="pc-cmt-body">
                  ...
                  <div className="pc-cmt-meta">
                    ...
                    {(currentUserId === c.user_id || isAdmin) && (
                      <button className="pc-cmt-del" onClick={() => delComment(c.id)}>
                        Delete
                      </button>
                    )}
                  </div>
                      ...
                      <div className="pc-cmt-body">
                        ...
                        <div className="pc-cmt-meta">
                          ...
                          {(currentUserId === r.user_id || isAdmin) && (
                            <button className="pc-cmt-del" onClick={() => delComment(r.id, c.id)}>
                              Delete
                            </button>
                          )}
                        </div>
        ...
        <div className="pc-overlay" onClick={() => setShowReport(false)}>
          <div className="pc-modal" onClick={(e) => e.stopPropagation()}>
            <div className="pc-modal-head">
              ...
              <button className="pc-modal-close" onClick={() => setShowReport(false)}>
                <X size={14} />
              </button>
            </div>
